 ...
 * @author Kumar Modified for RTPMT project 
 * @version 1.0
 */
public class SetupLoggingHook implements InitServerHook {
	...
	public void handleInit(QuickServer quickserver) throws Exception {
		...
		try	{
			...
			txtLog = new FileHandler("log/RTMPT_QuickServer%u%g.txt", 
				1024*1024, 20, true);
			...
			logger = Logger.getLogger("");
			txtLog = new FileHandler("log/RTMPTServer_Full%u%g.txt", 
				1024*1024, 20, true);
			txtLog.setFormatter(new SimpleTextFormatter());
			...
			txtLog = new FileHandler("log/RTMPTServer%u%g.txt", 
				1024*1024, 20, true);
			txtLog.setFormatter(new SimpleTextFormatter());
			txtLog.setLevel(Level.FINE);
			logger.addHandler(txtLog);
